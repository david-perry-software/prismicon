#!/usr/bin/env node
/**
 * Measure the Wright variant's static and animated cost.
 *
 * Static table: the five golden seeds plus the representative Wright seeds
 * (all four geometry families, all three palette families, one verified
 * hybrid) at size 64 — SVG bytes and the median paint() time over 200 calls,
 * gated at <= 32768 bytes and <= 5 ms median (`static gate: pass|fail`).
 *
...
const GENERIC_SEEDS = ['maya', 'build-bot-7', 'Alice@X.com', 'Ada Lovelace', 'demo-agent'];
// Representative Wright seeds: one per geometry family, one per palette family,
// and one verified hybrid (deriveWright('wright-hybrid-2').secondaryFamily === 'textile-block').
const WRIGHT_SEEDS = [
  ...
  'wright-hybrid-2'    // art-glass + textile-block
];
const SEEDS = [...GENERIC_SEEDS, ...WRIGHT_SEEDS];
const SIZE = 64;
...
const { renderStaticSVG } = createRenderer(createVariantRegistry([wright], { defaultId: 'wright' }));

const staticRows = SEEDS.map((seed) => {
  const svg = renderStaticSVG(seed, { variant: 'wright', size: SIZE });
  const bytes = Buffer.byteLength(svg, 'utf8');
  const derived = deriveWright(seed);
  const p = prepareWright(derived, { size: SIZE });
  const geo = buildWright(p);
  const pose = poseWright(p, 'idle');
  const times = [];
  for (let i = 0; i < PAINT_CALLS; i += 1) {
    const t0 = performance.now();
    paintWright(p, geo, pose, EFFECTS);
    times.push(performance.now() - t0);
  }
  const medianPaintMs = median(times);
  const pass = bytes <= MAX_BYTES && medianPaintMs <= MAX_MEDIAN_PAINT_MS;
  return { seed, family: derived.dominantFamily, palette: derived.paletteFamily, bytes, medianPaintMs, pass };
});

console.log(`wright static benchmark — spec ${WRIGHT_SPEC_VERSION}, size ${SIZE}, seeds: ${SEEDS.join(', ')}`);
...
console.log('');
console.log('| seed              | family        | palette        | bytes | median paint ms | pass |');
console.log('|-------------------|---------------|----------------|-------|-----------------|------|');
for (const r of staticRows) {
  console.log(`| ${r.seed.padEnd(17)} | ${r.family.padEnd(13)} | ${r.palette.padEnd(14)} | ${String(r.bytes).padStart(5)} | ${r.medianPaintMs.toFixed(3).padStart(15)} | ${r.pass ? 'yes ' : 'no  '} |`);
}
console.log('');
...
function simulateFrames(p, geo) {
  ...
  const frame = (state, transientT) => {
    t += FRAME_DT;
    const t0 = performance.now();
    pose = animateWright(pose, { params: p, state, dt: FRAME_DT, t, transientT, rest });
    paintWright(p, geo, pose, EFFECTS);
    times.push(performance.now() - t0);
  };
...
const frameRows = SEEDS.map((seed) => {
  const p = prepareWright(deriveWright(seed), { size: SIZE });
  const geo = buildWright(p);
...
console.log('');
console.log(`wright animated-frame benchmark — size ${SIZE}, dt ${FRAME_DT.toFixed(4)} s`);
...
for (const row of frameRows) {
  console.log(`| ${row.seed.padEnd(17)} | ${row.medianMs.toFixed(3).padStart(15)} | ${row.p95Ms.toFixed(3).padStart(12)} | ${String(row.settleFrames).padStart(13)} | ${row.pass ? 'yes ' : 'no  '} |`);
}
console.log('');
